...
3. Start a new `codex` TUI session (`vercel/bon-gour/gpt-oss-20b` first, then
...
| 5 | **Minimax footer** | Switch to `vercel/bon-gour/minimax-m2.5`, run the quicksort prompt, then `/quit`. Verify the TUI log contains the “Worked for …” separator before the final message and a `history/PlainHistoryCell` with the token usage footer. |
