...
- **Chromium model scouting:** When refreshing supported model lists, use an interactive Chromium harness (Playwright is acceptable) to browse `https://artificialanalysis.ai/` and extract current model names/slugs before updating allowlists.
- **Agentic index verification:** During model-refresh sweeps, run an interactive Chromium click-through from the Artificial Analysis homepage into the current agentic benchmark page (`/evaluations/gdpval-aa` at time of writing), capture full-page screenshots, and save a JSON evidence report confirming agentic-loop/tool-use wording before updating supported-model policy.
- **Observability first:** When behavior is unclear, add telemetry before attempting a fix. Instrument both the core (request/response) and the TUI so we can see what the model sent, what we forwarded, and how the UI rendered it. Never assume; capture the evidence in logs and reference it in `docs/CURRENT_TASK.md`. Use `python trace/telemetry.py …` to slice the JSONL sessions (`list`, `events`, `summary`, or `tui`) instead of scrolling giant logs by hand. Set `CODEX_TRACE_HOME` if you need to inspect an alternate `CODEX_HOME`. If the helper ever needs third-party deps, create `trace/venv` with `uv venv trace/venv` (preferred) or a standard virtualenv; the path is already gitignored. Legacy helpers under `scripts/` are considered deprecated—migrate any remaining use cases to the new tracing tool before touching them. Treat `trace/telemetry.py` as a living tool: whenever a new debugging scenario needs richer probes, extend the tracer first so future sweeps inherit the observability upgrade automatically.
- **TUI smoke checklist:** Before handing off a build or asking for user validation, run the manual telemetry checklist in `docs/TUI_TELEMETRY_CHECKLIST.md`. It enumerates the reasoning dedupe, buffered retry, git detection, and minimax footer scenarios we’ve hit so far. Each failure must be investigated via the new `logs/codex-tui-stream.jsonl` log (slice it with `python trace/telemetry.py tui …`) before the build can move forward. The goal is that reviewers can replay the *exact* TUI view from telemetry alone—do **not** rely on copy/pasted transcripts when validating formatting bugs.
...
- **Milestone releases + CI:** Push milestone branches/releases to GitHub at meaningful checkpoints and verify GitHub Actions runners start automatically and finish green. If a runner stalls/fails, record it in `docs/CURRENT_TASK.md` and fix or rerun before handoff.
...
- **All work happens in worktrees:** Never hack directly inside the primary `codex-litellm` checkout. For every new feature, fix, or spike, create a sibling worktree via `git worktree add ../codex-litellm-<slug> <base-branch>`, then point the Codex CLI session at that directory.
- **Resume ongoing efforts in place:** When returning to an unfinished task, reopen the Codex session with `cwd` set to the existing worktree instead of creating a fresh one. Log the active worktree path in `docs/CURRENT_TASK.md` so everyone knows where to pick up.
...
-   **Post-Milestone Publish Check:** After every milestone commit, explicitly ask the user whether to publish to npm immediately before continuing with additional release actions.
-   **npm Version Sync (Do this every release):** Before tagging/publishing, ensure `package.json` matches the checked-out upstream Codex build. Set `version` and `codexLitellm.baseVersion` to the upstream CLI version (from `cd codex && git describe --tags --exact-match HEAD` / `codex --version`), set `codexLitellm.upstreamCommit` to `cd codex && git rev-parse --short=8 HEAD`, and update `codexLitellm.releaseTag` to `vX.Y.Z-litellm.dev`. Then regenerate `package-lock.json` with `npm install --package-lock-only --ignore-scripts`.
